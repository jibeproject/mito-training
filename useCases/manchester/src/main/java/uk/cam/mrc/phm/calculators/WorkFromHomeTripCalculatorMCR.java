...
public class WorkFromHomeTripCalculatorMCR implements TripGenPredictor {

    private final DataSet dataSet;
    private final Random rand;
    private final Map<MitoPerson, Boolean> isMobile = new HashMap<>();

    public WorkFromHomeTripCalculatorMCR(DataSet dataSet) {
        this.dataSet = dataSet;
        rand = MitoUtil.getRandomObject();
        dataSet.getPersons().values().forEach(p -> isMobile.put(p,rand.nextDouble() <0.5));
